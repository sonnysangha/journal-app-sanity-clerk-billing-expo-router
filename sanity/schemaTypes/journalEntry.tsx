...
export const journalEntry = defineType({
  ...
  fields: [
    defineField({
      ...
      validation: (Rule) => [
        Rule.max(100).warning("Consider keeping titles under 100 characters for better readability")
      ]
    }),
    defineField({
      name: "content",
    ...
    defineField({
      ...
      validation: (Rule) => [
        Rule.required().error("Please select your mood for this entry")
      ]
    }),
    defineField({
      name: "userId",
      ...
      validation: (Rule) => [
        Rule.required().error("User ID is required for authentication")
      ]
    }),
    defineField({
    ...
    defineField({
      ...
      validation: (Rule) => [
        Rule.required().error("Creation date is required")
      ]
    })
  ],
  preview: {
    ...
    prepare({ title, content, mood, createdAt }: {
      title?: string;
      content?: any[];
      mood: string;
      createdAt: string;
